...
`docker-compose.prod.yml` deliberately does the same via the startup
...
`docker-compose.prod.yml` controls this.
...
- **Redis**: `redis-server --appendonly yes` is already set in
  `docker-compose.prod.yml`. Redis here is real-time plumbing and rate-
...
- [ ] TLS configured (nginx.conf.example is a starting point, not a
